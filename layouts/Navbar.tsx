  ...
  return (
    <section className="sticky top-0 z-40 bg-white bg-opacity-60 backdrop-blur-sm">
      <header className="w-full px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex justify-end items-center gap-1 hidden lg:flex flex-grow"> {/* ✅ Pushed fully right */}
          {LINKS.map((item, idx) => (
            <Link key={idx} href={item.link}>
              <NavButton className={pathname === item.link ? "text-[#f39e2f]" : "text-black"}>
                {item.label}
              </NavButton>
            </Link>
          ))}
          ...
          <Sheet open={isOpen} onOpenChange={setIsOpen}>
            ...
            <SheetContent side="right" className="text-slate-200 transition-all">
              <SheetHeader>
                ...
                <SheetDescription className="flex flex-col items-center pt-12 gap-2">
                  {LINKS.map((item, idx) => (
                    <Link key={idx} href={item.link} onClick={() => setIsOpen(false)}>
