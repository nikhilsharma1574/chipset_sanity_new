...
export const ContainerScroll = ({ titleComponent, notice }: { titleComponent: string | React.ReactNode; notice: Notice[]; }) => {
  ...
  const scaleDimensions = () => { return isMobile ? [0.9, 1.0] : [1.05, 1]; };
  ...
  return (
    <div className="h-[60rem] md:h-[80rem] flex items-center justify-center relative p-2 md:p-20" ref={containerRef}>
      <div className="py-10 md:py-10 w-full relative" style={{ perspective: "1000px" }}>
        <Header translate={translate} titleComponent={titleComponent} />
        <Card rotate={rotate} translate={translate} scale={scale} notice={notice} />
      </div>
    </div>
  );
};
